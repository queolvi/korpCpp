...
bool Month::operator==(const Month& rhs) const { m == rhs.m; }
bool Month::operator!=(const Month& rhs) const { !(*this == rhs); }
...
const Date& Date::AddMonths(int months) {
    ...
    for(int i = 0; i < months; ++i) {
        ++month_;
        if(month_ = Month::Jan()) year_ = Year(year_.GetYear() + 1);
        if(day_.GetDay() > month_.GetMaxDays(year_)) day_ = month_.GetMaxDays(year_);
...
void Month::dump (std::ostream& os) {
    os << m;
}

void Date::dump (std::ostream& os) {
    os << day_.GetDay() << '/' << month_ << '/' << year_.GetYear(); 
}
